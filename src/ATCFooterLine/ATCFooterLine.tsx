...
export const ATCFooterLine = () => {
  useEffect(() => {
    const updateRefillText = () => {
      ...
      const selectedBundle = label.classList[0];

      // 1. Ẩn tất cả <p>
      document
        .querySelectorAll<HTMLElement>('.product_atc-refills')
        .forEach((p) => {
          p.classList.add('hided');
        });

      // 2. Hiện đúng <p> tương ứng
      const target = document.querySelector<HTMLElement>(
        `.product_atc-refills.${selectedBundle}`,
      );
    ...
    const radios = document.querySelectorAll<HTMLInputElement>(
      'input[name="bundle-select"]',
    );

    radios.forEach((radio) => {
      radio.addEventListener('change', updateRefillText);
    });

    // cleanup khi unmount component
    return () => {
      radios.forEach((radio) => {
        radio.removeEventListener('change', updateRefillText);
      });
    };
  }, []);
  return (
    <div className="atc-footer-line mt-[16px] flex justify-center items-center gap-[16px]">
      <div className="atc-footer-inline flex gap-[8px] pe-[16px] border-r border-black">
        <div>
          <svg
        ...
        <div className="font-(family-name:--body-font) text-[12px]">
          <p className="product_atc-refills bundle__single-item">
            Refills Ship Monthly
          </p>
          <p className="product_atc-refills bundle__buy-two hided">
            Refills Ship Bi-Monthly
          </p>
          <p className="product_atc-refills bundle__buy-three hided">
            Refills Ship Every 12 Week
          </p>
        </div>
      </div>
      <div className="atc-footer-inline flex gap-[8px]">
